...
(function () {
  ...
  async function wallet() {
    ...
    throw Error('Connect and verify a Cardano wallet first. Dreaming Anime will not restore a wallet silently.');
  }
  ...
  async function open(product, name) { ensurePanel(); activeProduct = product; intent = null; var panel = document.getElementById('da-ada-panel'), status = document.getElementById('da-ada-status'), payButton = document.getElementById('da-ada-pay'), net = document.getElementById('da-ada-network'); panel.classList.add('open'); payButton.disabled = true; status.textContent = 'Preparing your secure Preprod checkout…';
    try { await loadCsl(); var api = await wallet(); var network = typeof api.getNetworkId === 'function' ? await api.getNetworkId() : await api.getNetwork(); if (Number(network) !== 0) throw Error('Checkout blocked to protect your mainnet funds. Switch the connected wallet to Cardano Preprod.'); net.textContent = 'Preprod wallet detected.'; intent = await createIntent(product); payButton.disabled = false; status.textContent = name + ' · payment intent ready. Approve exactly 8 ADA in your wallet.'; } catch (e) { status.textContent = e.message || 'Checkout could not start.'; }
  }
  ...
  document.addEventListener('click', function (event) { var button = event.target.closest('[data-ada-product]'); if (!button) return; event.preventDefault(); open(button.dataset.adaProduct, button.dataset.adaName || 'Wallpaper pack'); });
