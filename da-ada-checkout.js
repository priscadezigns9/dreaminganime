...
(function () {
  ...
  var CSL_URL = 'https://unpkg.com/@emurgo/cardano-serialization-lib-asmjs@15.0.3/cardano_serialization_lib.js';
  var ADA = 8000000;
  ...
  async function open(product, name) { ensurePanel(); activeProduct = product; intent = null; var stage='opening checkout'; var panel = document.getElementById('da-ada-panel'), status = document.getElementById('da-ada-status'), payButton = document.getElementById('da-ada-pay'), connectButton = document.getElementById('da-ada-connect'), net = document.getElementById('da-ada-network'); panel.classList.add('open'); payButton.disabled = true; connectButton.hidden = true; status.textContent = 'Preparing your secure Preprod checkout…';
    try { stage='loading Cardano transaction support'; await loadCsl(); stage='checking wallet API'; var api = await wallet(); stage='checking wallet network'; if (Number(await api.getNetworkId()) !== 0) throw Error('Checkout blocked to protect your mainnet funds. Switch the connected wallet to Cardano Preprod.'); net.textContent = 'Preprod wallet detected.'; connectButton.hidden = true; stage='creating payment intent'; intent = await createIntent(product); payButton.disabled = false; status.textContent = name + ' · payment intent ready. Approve exactly 8 ADA in your wallet.'; } catch (e) { var msg=e.message||'Checkout could not start.'; if(msg==='Failed to fetch') msg='The secure checkout service could not be reached. Please retry once.'; status.textContent = (msg==='The secure checkout service could not be reached. Please retry once.'?'Checkout failed during '+stage+': '+msg:msg); connectButton.hidden = /wallet|injected|connect/i.test(msg); }
  }
  async function pay() { var status = document.getElementById('da-ada-status'), button = document.getElementById('da-ada-pay'), payStage='preparing transaction'; button.disabled = true; try { if (!intent) throw Error('Payment intent is not ready.'); payStage='checking wallet'; var api = await wallet(); payStage='building transaction'; var tx = await buildTx(api, intent.receiving_address); status.textContent = 'Review and approve the exact 8 ADA payment in your wallet…'; payStage='waiting for wallet approval'; var signed = await api.signTx(tx); payStage='submitting transaction'; var hash = await api.submitTx(signed); status.textContent = 'Transaction submitted. Verifying payment…'; payStage='verifying payment'; var response = await fetch(ENDPOINT + 'da-verify-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ paymentIntentId: intent.id || intent.payment_intent_id, transactionHash: hash }) }); var body = await response.json(); if (body.status === 'confirmed') status.textContent = 'Payment confirmed. Your wallpaper entitlement is ready.'; else if (response.status === 202 || body.status === 'pending') status.textContent = 'Payment pending confirmation. Keep this page open and verify again shortly.'; else throw Error(body.error || 'Payment verification failed.'); } catch (e) { var msg=e.message||'Payment was not completed.'; status.textContent=msg==='Failed to fetch'?'Checkout failed during '+payStage+': '+msg:msg; } finally { button.disabled = !intent; } }
