...
(function(){function restore(){window.daWalletApi=null;localStorage.removeItem('dreamingAnimeWalletProvider');localStorage.removeItem('dreamingAnimeWalletAddress');}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',restore);else restore()})();

(function(){function pill(){if(document.getElementById('da-universal-pill-buttons'))return;var st=document.createElement('style');st.id='da-universal-pill-buttons';st.textContent='button,.button,.btn,input[type=button],input[type=submit],a.button,a.btn{border-radius:999px!important;white-space:nowrap}button,.button,.btn,input[type=button],input[type=submit],a.button,a.btn{margin:6px!important}';document.head.appendChild(st)}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',pill);else pill()})();

(function(){function grids(){if(document.getElementById('da-five-column-grids'))return;var st=document.createElement('style');st.id='da-five-column-grids';st.textContent='@media(min-width:1200px){.cards,.card-grid,.products-grid,.gallery,.shop-grid{grid-template-columns:repeat(5,minmax(0,1fr))!important}}@media(min-width:760px) and (max-width:1199px){.cards,.card-grid,.products-grid,.gallery,.shop-grid{grid-template-columns:repeat(3,minmax(0,1fr))!important}}@media(max-width:759px){.cards,.card-grid,.products-grid,.gallery,.shop-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important}}';document.head.appendChild(st)}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',grids);else grids()})();

(function(){function install(){var b=document.querySelector('.da-connect-wallet'),ws=b&&b.nextElementSibling;if(!b||!ws||document.getElementById('da-wallet-disconnect'))return;if(ws.dataset.connected==='true'||ws.textContent.indexOf('NOT CONNECTED')<0&&(ws.textContent.indexOf('CONNECTED')>=0||ws.textContent.indexOf('WALLET ENABLED')>=0)){var d=document.createElement('button');d.id='da-wallet-disconnect';d.type='button';d.textContent='DISCONNECT WALLET';d.className='da-connect-wallet';d.addEventListener('click',function(){localStorage.removeItem('dreamingAnimeWalletProvider');localStorage.removeItem('dreamingAnimeWalletAddress');localStorage.removeItem('dreamingAnimeSession');localStorage.removeItem('dreamingAnimeAccountId');window.daWalletApi=null;b.textContent='CONNECT WALLET';ws.textContent='NOT CONNECTED';ws.dataset.connected='false';d.remove()});b.parentNode.insertBefore(d,b.nextSibling)}}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install);else install();new MutationObserver(install).observe(document.documentElement,{subtree:true,childList:true,characterData:true})})();

(function(){function navSize(){if(document.getElementById('da-equal-nav-controls'))return;var st=document.createElement('style');st.id='da-equal-nav-controls';st.textContent='nav a:not(.brand):not(.nav-brand),nav button,nav .da-connect-wallet,nav #da-wallet-disconnect{width:128px!important;min-width:128px!important;height:44px!important;min-height:44px!important;padding:0 8px!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;font-size:.62rem!important}';document.head.appendChild(st)}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',navSize);else navSize()})();

(function(){function setupChooser(){var old=document.querySelector('.da-connect-wallet');if(!old||old.dataset.walletChooserBound)return;var b=old.cloneNode(true);b.dataset.walletChooserBound='true';old.replaceWith(b);b.addEventListener('click',function(e){e.preventDefault();var w=window.cardano||{},keys=Object.keys(w).filter(function(k){return w[k]&&typeof w[k].enable==='function'});var names={vespr:'Vespr',lace:'Lace',eternl:'Eternl',nami:'Nami',flint:'Flint',yoroi:'Yoroi',typhon:'Typhon'};var overlay=document.createElement('div');overlay.id='da-wallet-chooser';overlay.innerHTML='<div class="da-wallet-sheet"><button class="da-wallet-close" type="button">CLOSE</button><h2>Choose a Cardano wallet</h2><p>Select the wallet you want to connect.</p><div class="da-wallet-options">'+(keys.length?keys.map(function(k){return '<button type="button" data-wallet="'+k+'">'+(names[k]||k.toUpperCase())+'</button>'}).join(''):'<p>No supported Cardano wallet was detected.</p>')+'</div></div>';document.body.appendChild(overlay);overlay.querySelector('.da-wallet-close').onclick=function(){overlay.remove()};overlay.querySelectorAll('[data-wallet]').forEach(function(opt){opt.onclick=async function(){var key=opt.dataset.wallet;opt.disabled=true;opt.textContent='CONNECTING…';try{var api=window.daNormalizeCip30?window.daNormalizeCip30(await w[key].enable()):await w[key].enable();var used=await api.getUsedAddresses(),address=used[0]||await api.getChangeAddress();if(!address)throw Error('Wallet returned no address');window.daWalletApi=api;var status=b.nextElementSibling;if(status){status.textContent='WALLET ENABLED · CHECKOUT WILL VERIFY';status.dataset.connected='false'}b.textContent=(names[key]||key.toUpperCase())+' ENABLED';overlay.remove()}catch(err){opt.disabled=false;opt.textContent=names[key]||key.toUpperCase();var msg=overlay.querySelector('.da-wallet-sheet p');msg.textContent=err.message||'Wallet connection failed.'}}})})}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',setupChooser);else setupChooser()})();

(function(){var st=document.createElement('style');st.textContent='#da-wallet-chooser{position:fixed;inset:0;z-index:10000;background:rgba(0,0,0,.75);display:grid;place-items:center;padding:20px}.da-wallet-sheet{width:min(420px,100%);background:#151922;color:#fff;border:1px solid #ff7a18;border-radius:22px;padding:24px;box-shadow:10px 12px 0 #5b260b}.da-wallet-sheet h2{color:#ff7a18;margin-top:0}.da-wallet-sheet button{width:100%;min-height:46px;margin:7px 0;border-radius:999px;border:1px solid #ff7a18;background:#24242b;color:#fff;font-weight:700}.da-wallet-sheet .da-wallet-close{background:#ff7a18;color:#111}';document.head.appendChild(st)})();

window.daOpenWalletChooser=function(){var b=document.querySelector(".da-connect-wallet");if(b)b.click();};
