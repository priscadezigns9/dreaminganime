...
(function(){function bind(){var b=document.querySelector('.da-connect-wallet');if(!b||b.dataset.bound)return;b.dataset.bound='true';b.removeAttribute('href');b.setAttribute('role','button');b.addEventListener('click',async function(e){e.preventDefault();try{var endpoint='https://sazhdnqzaqpqcralmthh.supabase.co/functions/v1/da-wallet-auth',w=window.cardano||{},keys=Object.keys(w).filter(function(k){return w[k]&&typeof w[k].enable==='function'}),key=['vespr','lace','eternl','nami','flint','yoroi','typhon'].find(function(k){return keys.indexOf(k)>=0})||keys[0];if(!key){b.textContent='INSTALL WALLET';return}b.textContent='CONNECTING…';var api=await w[key].enable(),used=await api.getUsedAddresses(),address=used[0]||await api.getChangeAddress(),nr=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'nonce',chain:'cardano',walletAddress:address})}),n=await nr.json();if(!nr.ok)throw Error(n.error||'Nonce request failed');b.textContent='SIGN IN '+key.toUpperCase();var sig=await api.signData(address,n.payload),vr=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'verify',chain:'cardano',walletAddress:address,nonce:n.nonce,key:sig.key,signature:sig.signature,provider:key})}),r=await vr.json();if(!vr.ok)throw Error(r.error||'Signature verification failed');localStorage.setItem('dreamingAnimeSession',r.sessionToken);localStorage.setItem('dreamingAnimeAccountId',r.accountId);b.textContent=key.toUpperCase()+' CONNECTED';}catch(err){b.textContent='CONNECT WALLET';console.warn(err)}})}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind);else bind()})();
