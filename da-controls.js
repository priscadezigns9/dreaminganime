...
function daNormalizeCip30(candidate){var api=candidate&&candidate.api&&typeof candidate.api==='object'?candidate.api:candidate;var required=['getNetwork','getUtxos','getChangeAddress','signTx','submitTx'];var missing=required.filter(function(k){return typeof (api&&api[k])!=='function'});if(missing.length)throw Error('Wallet API compatibility error: missing '+missing.join(', '));return api;}
