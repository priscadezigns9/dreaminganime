...
(function () {
  ...
  var controlCSS = '.da-brand-logo{width:48px;height:48px;border-radius:50%;object-fit:cover;display:inline-block;vertical-align:middle;margin-right:12px;border:2px solid #FF6B00}.da-brand-lockup{display:inline-flex;align-items:center}' + '.float-stack{position:fixed;bottom:28px;right:24px;display:flex;flex-direction:column;gap:12px;z-index:900;align-items:center}.float-btn{width:52px;height:52px;border-radius:50%;display:flex;align-items:center;justify-content:center;text-decoration:none;border:2px solid #0A0A0A;transition:transform .2s,box-shadow .2s;font-size:22px;box-shadow:3px 3px 0 #0A0A0A}.float-btn:hover{transform:translateY(-3px);box-shadow:5px 5px 0 #0A0A0A}.float-top{background:#FF6B00;color:#FFF;font-family:Bangers,cursive;font-size:20px;cursor:pointer}.float-translate{background:#FFF;border-radius:24px;padding:0 16px;height:38px;width:auto;font-family:"Space Mono",monospace;font-size:10px;font-weight:700;color:#0A0A0A;letter-spacing:.06em;white-space:nowrap;cursor:pointer}@media(max-width:600px){.float-stack{right:14px;bottom:18px;gap:8px}.float-translate{font-size:9px;padding:0 12px}.float-btn{box-shadow:2px 2px 0 #0A0A0A}}';
  function addBrandLogo() { var host=document.querySelector('.nav, nav, .navbar, header'); if(!host || host.querySelector('.da-brand-logo')) return; var img=document.createElement('img'); img.className='da-brand-logo'; img.src='/logos/dreaming-anime-logo-round.png'; img.alt='Dreaming Anime'; var first=host.querySelector('a, .logo'); if(first){ first.parentNode.insertBefore(img,first); } else { host.insertBefore(img,host.firstChild); } }
  function init() {
    addBrandLogo();
